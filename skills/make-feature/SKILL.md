...
4. **Add Git Worktree**:
   * If creating a new branch, run:
     ```bash
     git worktree add -b gemini/<feature-name> <repo_root>/tmp/worktrees/gemini_<feature-name> origin/<base_branch>
     ```
   * If checking out an existing local or remote branch, run:
     ```bash
     git worktree add <repo_root>/tmp/worktrees/gemini_<feature-name> gemini/<feature-name>
     ```
5. **Modify & Develop**: Perform all file edits, writes, and local commands inside the isolated worktree directory (`<repo_root>/tmp/worktrees/gemini_<feature-name>`). Do not make changes in the primary workspace.
...
7. **Push Branch**: Push the feature branch to the remote origin:
   ...
   ```
8. **Clean Up**: Remove the worktree from the primary repository directory:
   ```bash
   git worktree remove <repo_root>/tmp/worktrees/gemini_<feature-name>
   ```
