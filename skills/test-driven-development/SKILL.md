...
description: Drives development with tests. Use when implementing logic, fixing bugs, or changing behavior to prove correctness.
---
...
When executing under `/make-feature` Phase 2 (Build, Worktree & RED Test Remote Push Gate) or Heavy Mode per-slice flow, parent agent invokes `invoke_subagent` (`TypeName: self`, `Role: Adversarial Test Reviewer`). Subagent audits written RED tests to verify:
...
- 100% parity with requirements and edge cases defined in the spec (in-tree `${FEATURE_SLUG}/spec.md` under `make-feature`).
