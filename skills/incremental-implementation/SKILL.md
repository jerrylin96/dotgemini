...
description: Guidelines to deliver code changes incrementally. Use when implementing changes spanning multiple files to avoid large commits.
---
...
4. **Commit** -- save your progress with a descriptive message (use atomic commits and clear commit messages)
...
> [!TIP]
> **Subagent Delegation**: For **Contract-First Slicing**, the main agent can define the contracts/interfaces (Slice 0) and invoke concurrent, built-in `self` subagents using `Workspace: branch` (isolated workspace directory cloned or branched from the parent) to build the backend (Slice 1a) and frontend (Slice 1b) concurrently.
...
> 1. **Workspace Mode**: Must use `Workspace: inherit` (or target worktree path) so each subagent commits directly onto the shared feature branch (`gemini/<feature-name>-<hash>`).
...
> 7. **Operational Conventions & Triggers**: Recommended in `/plan` for 5 or more complex multi-file slices, or selected when the user provides intent signals in prompts or command invocation conventions (`/plan heavy`, `/make-feature heavy`, or explicit handoff directives), where the agent proactively selects `Sequential Subagents` execution strategy.
