...
- `temp_commit_stat.txt`: Stores per-commit changed-file statistics (`git show --stat --format="" -M ...`).
- `temp_commit_diff.txt`: Stores per-commit per-file diff hunks (`git show -M <sha> -- <file>`).
- `temp_diff_stat.txt`: Stores cumulative changed-file statistics, used for topic-by-topic clustering and high-level changeset partitioning (`git diff ... --stat -M`).
- `temp_diff_numstat.txt`: Stores machine-readable line insertion/deletion totals (`git diff ... --numstat -z -M`), used for precise text line reconciliation.
- `temp_diff_all.txt`: Stores complete cumulative diff hunks and context (`git diff ... -M`).
- `temp_diff.txt`: Stores the per-file cumulative walkthrough diff. Dynamically extract each file's diff to a single stable location, overwriting it for each selected file (`git diff ... -M -- <file>`).
- `temp_diff_paths.txt`: Optional scratch file generated only for large changesets. Stores null-delimited name-status list (`git diff ... --name-status -z -M`), used for path/status enumeration and large diff topic clustering. Must only be read when generated.
...
- **Explicit Rename Detection Flag (`-M`)**: Always specify explicit `-M` across all diff artifact commands (`--stat -M`, `--numstat -z -M`, `--name-status -z -M`, and full diff `-M`) so rename/copy detection is deterministic and immune to differing local Git configurations (`diff.renames`).

## 4. Special Git Cases & Changed-File Accounting
- **Renames & Copies**:
  - In `git diff --stat -M`, a rename counts as **1 changed file** (e.g., `old => new | 0`).
  - In `git diff --name-status -z -M`, a rename produces: `R<score>\0<old_path>\0<new_path>\0`.
  - In `git diff --numstat -z -M`, a rename produces: `<added>\t<deleted>\t\0<old_path>\0<new_path>\0`.
...
Both `temp_diff_paths.txt` (`--name-status -z -M`) and `temp_diff_numstat.txt` (`--numstat -z -M`) emit NUL-byte (`\0`) delimited streams to guarantee safety against file paths containing spaces, quotes, newlines, or Unicode characters.
- **Parsing `--name-status -z -M`**:
