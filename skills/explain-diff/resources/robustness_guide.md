...
- `temp_topic_diff.txt`: Stores path-limited topic diff hunks for topic-by-topic walkthrough (`git diff ... --find-renames --find-copies -- "<file1>" "<file2>" > temp_topic_diff.txt`).
...
- **Allowed Deletions (Exception)**: Exact, agent-created temporary files and directories under the verified conversation scratch directory (e.g. `<appDataDir>/brain/<conversation-id>/scratch/`, including `temp_commits.txt`, `temp_diff_stat.txt`, `temp_diff_numstat.txt`, `temp_diff_all.txt`, `temp_diff.txt`, `temp_diff_paths.txt`, `temp_topic_diff.txt`) are an explicit exception and can be safely deleted, even when `<appDataDir>` resides beneath the cloned configuration repository path.
...
## 8. Truncation Handling & Grounded Quotation Invariants
- **Terminal Truncation & Hallucination Risk**: When command outputs (such as raw `git diff` or ad-hoc terminal scripts like `python3 -c`) dump bulk diff hunks directly to terminal stdout, the terminal runtime silently truncates output (`<truncated N lines>`, `<truncated N bytes>`, `observation too long`, or `stdout_truncated: true`). When the model encounters a truncated output gap, it is prone to generative hallucination—inventing plausible-looking diff hunks, code lines, or author/metadata citations from parametric memory rather than grounded reality.
- **Mechanical Path-Scoped Extraction**: To eliminate stdout truncation risk, agents must never dump bulk diffs directly to stdout. Instead, mechanically extract path-limited diffs into targeted scratch files (`temp_topic_diff.txt` for topics, `temp_commit_diff.txt` for commits, `temp_diff.txt` for individual files) with explicit rename and copy detection (`--find-renames --find-copies`).
- **Grounded Quotation Invariant**: Emitting any fenced `diff` block without prior `view_file` inspection in active context is strictly forbidden. The agent must verify command exit status 0 (fail closed on non-zero), determine total line count, and iteratively page through EOF via `view_file` (up to 800 lines per turn) before quoting hunks verbatim.
- **Binary & Non-Text Metadata**: For entries where `numstat` reports `-\t-\t` (binary files, submodules, symlinks, mode changes), use metadata tags per §4; do not attempt to render binary diffs in text fenced blocks.
