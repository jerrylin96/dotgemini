...
> [!TIP]
> **Subagent Delegation (Antigravity Only)**: If the changeset is exceptionally large (many files or large diffs), the main agent should delegate the task. Invoke the built-in `research` subagent to analyze the diff chunks in the background and draft the overall summary and file-by-file gists. In runtimes without subagent support (e.g. Gemini CLI), the main agent must perform these steps directly.
