...
1. **Get the Diff Safely**: To prevent terminal command output truncation (which silently trims long diff outputs or lines), do NOT read the raw output of `git diff` directly from the terminal tool. Instead:
   ...
   *Execution and Robustness Guidelines:*
   ...
   - **Machine-Readable Path Enumeration**: To safely handle renames, whitespace, special characters, and newlines in filenames, run `git diff <reference_commit_hash>...<commit_hash> --name-status -z` to retrieve a null-byte-terminated list of changed paths and rename pairs. Parse this machine-readable list carefully.
   ...
   - **Pagination & EOF Detection**: To prevent terminal output truncation on extremely large files, read files in successive, deterministic chunks. Do not rely on receiving a short chunk (fewer lines than requested) as an EOF signal. Instead, get the total line count of the file beforehand (e.g. via `wc -l "<file_path>"` or file viewer metadata) and read iteratively until the `StartLine` exceeds the total line count.
   ...
   - **Interactive Phase & Cleanup**: Preserve the temp files (`temp_diff_all.txt` and `temp_diff.txt`) during the interactive follow-up phase to allow revisiting files. Once the walkthrough and follow-up Q&A end, cleanly delete only the temporary files and directories: run `rm -- "<file_path>"` for scratch files, or `rm -rf -- "$TEMP_DIR"` if an OS temporary directory was created via `TEMP_DIR=$(mktemp -d)` (first verifying `$TEMP_DIR` is not empty and resides under a valid temporary path like `/tmp` or `$TMPDIR`). Never run deletion commands inside the repository or worktree.
