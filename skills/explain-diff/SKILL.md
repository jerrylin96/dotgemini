...
1. **Get the Diff Safely & Extract Commits**: To prevent terminal command output truncation (which silently trims long diff outputs or lines), do NOT read raw git outputs directly from the terminal. Instead:
   ...
   c. **Write Statistics**: Save the changed-file statistics with explicit rename and copy detection (`--find-renames --find-copies`):
      - Normal commit/branch range:
        `git diff "<reference_commit_hash>...<commit_hash>" --stat --find-renames --find-copies > "<appDataDir>/brain/<conversation-id>/scratch/temp_diff_stat.txt"`
      - Root commit (where `<reference_commit_hash>` is the empty tree `4b825dc642cb6eb9a060e54bf8d69288fbee4904`):
        `git diff "<reference_commit_hash>" "<commit_hash>" --stat --find-renames --find-copies > "<appDataDir>/brain/<conversation-id>/scratch/temp_diff_stat.txt"`
   d. **Write Numstat Line Totals**: Save machine-readable NUL-delimited line and file totals with explicit rename and copy detection (`--find-renames --find-copies`):
      - Normal commit/branch range:
        `git diff "<reference_commit_hash>...<commit_hash>" --numstat -z --find-renames --find-copies > "<appDataDir>/brain/<conversation-id>/scratch/temp_diff_numstat.txt"`
      - Root commit (empty tree):
        `git diff "<reference_commit_hash>" "<commit_hash>" --numstat -z --find-renames --find-copies > "<appDataDir>/brain/<conversation-id>/scratch/temp_diff_numstat.txt"`
   e. **Write Complete Diff**: Save the full diff output with explicit rename and copy detection (`--find-renames --find-copies`):
      - Normal commit/branch range:
        `git diff "<reference_commit_hash>...<commit_hash>" --find-renames --find-copies > "<appDataDir>/brain/<conversation-id>/scratch/temp_diff_all.txt"`
      - Root commit (empty tree):
        `git diff "<reference_commit_hash>" "<commit_hash>" --find-renames --find-copies > "<appDataDir>/brain/<conversation-id>/scratch/temp_diff_all.txt"`
   f. **Enumerate Paths**: For unusually large changesets, run null-delimited path/status enumeration with explicit rename and copy detection (`--find-renames --find-copies`):
      - Normal commit/branch range:
        `git diff "<reference_commit_hash>...<commit_hash>" --name-status -z --find-renames --find-copies > "<appDataDir>/brain/<conversation-id>/scratch/temp_diff_paths.txt"`
      - Root commit (empty tree):
        `git diff "<reference_commit_hash>" "<commit_hash>" --name-status -z --find-renames --find-copies > "<appDataDir>/brain/<conversation-id>/scratch/temp_diff_paths.txt"`
...
2. **Overall Summary & Topic Clustering**:
   ...
   - **Topic Clustering Protocol**:
     ...
     - **Coverage & Reconciliation Invariant**:
       ...
       - **Unique changed-file entity count ($U$)**: The number of distinct changed-file records matching `git diff --stat --find-renames --find-copies`'s summary ("N files changed"). A pure rename (`old_path -> new_path`) counts as **1 changed-file entity** in $U$ (not 2 paths). Copies, submodules, symlinks, and typechanges each count as **1 changed-file entity** in $U$.
       - **File-topic membership count ($M$)**: The sum of file references across topics. A file MAY belong to multiple topics when its hunks touch separate functional concerns; in such cases, $M \ge U$ and topic membership counts are not required to equal the unique changed-file entity count. A rename or copy contributes 1 to its topic's file list.
       ...
       - **Deterministic Submodule, Symlink & Typechange Accounting**: Metadata entity modifications (submodule commit pointer changes `160000`, symlink target modifications `120000`, and typechanges `T`) strictly follow the numerical values in `temp_diff_numstat.txt`: when numstat reports 0 or `-\t-\t<path>` (non-text), they contribute 0 to text line additions/deletions and their changes are reconciled under their respective metadata category tags (`[submodule: ...]`, `[symlink: ...]`, `[typechange: ...]`). If numstat records numerical text lines for them, those exact numstat line counts are assigned to the entity's topic so global and topic totals reconcile unconditionally.
     ...
     - **Binary, Deletion, Rename, Submodule & Symlink Handling**:
       - **Binary files**: Counted in topic file lists with explicit old-size/new-size byte formats derived from `git diff --stat --find-renames --find-copies` (e.g. `Bin old -> new bytes`) or blob inspection (`git cat-file -s` / `git ls-tree`): `[binary file: <path> (old: X bytes -> new: Y bytes)]` (or `[binary addition: <path> (new: Y bytes)]`). Binary bytes are kept strictly separate from text-line counts.
...
3. **Tri-Lens Navigation Menu**:
   ...
   ```text
   Summary: 3 topics across 6 unique files (+112 / -28)
...
5. **Commit-by-Commit Walkthrough Flow (`[c]`)**:
   ...
   b. **Commit File Stat**: Extract commit-specific file statistics with explicit rename and copy detection:
      `git show --stat --format="" --find-renames --find-copies "<sha>" > "<appDataDir>/brain/<conversation-id>/scratch/temp_commit_stat.txt"`
   c. **Hunk Inspection**: For each modified file in the commit, extract diff with explicit rename and copy detection:
      `git show --find-renames --find-copies "<sha>" -- "<file>" > "<appDataDir>/brain/<conversation-id>/scratch/temp_commit_diff.txt"`
...
6. **File-by-File Walkthrough Flow (`[f]`)**:
   ...
   a. Present numbered menu of changed files (path, `+/-` stats, hunk count, one-line gist) plus:
      ...
      - `[q]` finish.
   b. For the chosen file, write target diff to `temp_diff.txt` with explicit rename and copy detection (`--find-renames --find-copies`):
      - Normal commit/branch range:
        `git diff "<reference_commit_hash>...<commit_hash>" --find-renames --find-copies -- "<file>" > "<appDataDir>/brain/<conversation-id>/scratch/temp_diff.txt"`
      - Root commit (empty tree):
        `git diff "<reference_commit_hash>" "<commit_hash>" --find-renames --find-copies -- "<file>" > "<appDataDir>/brain/<conversation-id>/scratch/temp_diff.txt"`
      *(Note on path-limited diff representation: when diffing a single target path, if the source of a rename or copy is outside the pathspec, Git only inspects the target file. The agent relies on the global status record from `temp_diff_paths.txt` or `temp_diff_numstat.txt` to display the source relationship metadata tag, e.g. `[rename: <old_path> -> <new_path>]` or `[copy: <source_path> -> <target_path>]`).*
