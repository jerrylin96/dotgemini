...
Shell policy recognizes directly parsed commands beginning with `git merge`, `gh pr ... create`, or `git push` refspecs targeting the configured base branch. Shell wrappers, compound scripts, aliases, alternative Git clients, and other integration mutations are not comprehensively parsed. Never describe this as a shell sandbox.
